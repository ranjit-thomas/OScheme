...
and tokens_to_s_expr_func_call list_of_tokens =
	let (s_prod, remaining_tokens) = tokens_to_s_expr list_of_tokens in
	(*
	...
	let rec collect_args s_args list_of_tokens =
		...
		| _ -> 
			let (s_arg, remaining_tokens) = tokens_to_s_expr remaining_tokens in
			collect_args (s_arg :: s_args) remaining_tokens)
	in
	collect_args [] remaining_tokens
